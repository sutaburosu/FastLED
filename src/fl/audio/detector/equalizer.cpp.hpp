...
void EqualizerDetector::update(shared_ptr<Context> context) {
    ...
    // Use Context's cached fft::FFT (shared across detector)
    ...
    for (int i = 0; i < numBins; ++i) {
        // Step 1: Copy raw and apply fft::FFT downscale
        ...
        // Step 4: Apply fft::FFT scaling mode
        if (applyScalingMode) {
    ...
    // Zero-crossing factor (already 0.0-1.0 from Sample)
    mZcf = context->getZCF();
