...
FramePtr FileSystem::loadJpeg(const char *path, const JpegConfig &config,
    ...
    if (!file.is_open()) {
        ...
        FASTLED_WARN("Failed to open JPEG file: " << path); return FramePtr();
    }
    ...
    if (bytesRead != fileSize) {
        ...
        FASTLED_WARN("Failed to read complete JPEG file: " << path); return FramePtr();
    }
...
fl::Mp3DecoderPtr FileSystem::openMp3(const char *path, fl::string *error_message) {
    ...
    if (!file.is_open()) {
        if (error_message) { *error_message = "Failed to open file: "; error_message->append(path); }
        FASTLED_WARN("Failed to open MP3 file: " << path); return fl::Mp3DecoderPtr();
    }
    ...
    if (!decoder->begin(file.rdbuf())) {
        ...
        FASTLED_WARN("Failed to initialize MP3 decoder for: " << path); return fl::Mp3DecoderPtr();
    }
    return decoder;
}
