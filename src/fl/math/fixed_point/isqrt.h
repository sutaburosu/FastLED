...
FL_OPTIMIZE_FUNCTION constexpr inline u64 _isqrt64_start(u64 x, u64 bit) {
    return (bit == 0 || bit <= x) ? bit : _isqrt64_start(x, bit >> 2);
}

FL_OPTIMIZE_FUNCTION constexpr inline u32 _isqrt64_step(u64 x, u64 result, u64 bit) {
    return bit == 0
...
FL_OPTIMIZE_FUNCTION constexpr inline u32 _isqrt32_start(u32 x, u32 bit) {
    return (bit == 0 || bit <= x) ? bit : _isqrt32_start(x, bit >> 2);
}

FL_OPTIMIZE_FUNCTION constexpr inline u16 _isqrt32_step(u32 x, u32 result, u32 bit) {
    return bit == 0
...
FL_OPTIMIZE_FUNCTION constexpr inline u16 isqrt32(u32 x) {
    return x == 0 ? u16(0)
                  : _isqrt32_step(x, 0, _isqrt32_start(x, u32(1) << 30));
}

FL_OPTIMIZE_FUNCTION constexpr inline u32 isqrt64(u64 x) {
    return x == 0 ? u32(0)
