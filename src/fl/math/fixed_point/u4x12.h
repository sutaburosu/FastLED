...
class u4x12 {
  public:
    ...
    explicit constexpr u4x12(IntT n)
    ...
    friend constexpr u4x12 operator*(u16 scalar, u4x12 fp) {
    ...
    static constexpr FASTLED_FORCE_INLINE u4x12 sqrt(u4x12 x) {
        return x.mValue == 0 ? u4x12() : from_raw(static_cast<u16>(
            fl::isqrt32(static_cast<u32>(x.mValue) << FRAC_BITS)));
    }

    static constexpr FASTLED_FORCE_INLINE u4x12 rsqrt(u4x12 x) {
