...
bool HttpStreamClient::sendHttpRequestHeader() {
    ...
    // Send header
    int sent = mNativeClient->send(fl::span<const u8>(reinterpret_cast<const u8*>(header.c_str()), header.size())); // ok reinterpret cast
...
bool HttpStreamClient::readHttpResponseHeader() {
    ...
    while (headerBuffer.size() < MAX_HEADER_SIZE) {
        int received = mNativeClient->recv(buffer);
