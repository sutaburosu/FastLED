...
int HttpStreamServer::recvData(fl::span<u8> buffer) {
    ...
    for (size_t offset = 0; offset < clientIds.size(); offset++) {
        ...
        int received = mNativeServer->recv(clientId, buffer);
        if (received > 0) {
...
bool HttpStreamServer::sendHttpResponseHeader(u32 clientId) {
    ...
    // Send header
    int sent = mNativeServer->send(clientId, fl::span<const u8>(reinterpret_cast<const u8*>(header.c_str()), header.size())); // ok reinterpret cast
