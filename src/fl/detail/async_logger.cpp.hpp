...
namespace detail {
    ...
    struct BackgroundFlushState {
        ...
        fl::isr::handle mTimerHandle; // ISR timer handle
...
bool AsyncLogger::enableBackgroundFlush(fl::u32 interval_ms, fl::size messages_per_tick) {
    ...
    if (fl::isr::attach_timer_handler(config, &state.mTimerHandle) != 0) {
...
void AsyncLogger::disableBackgroundFlush() {
    ...
    if (state.mEnabled && state.mTimerHandle.is_valid()) {
        fl::isr::detach_handler(state.mTimerHandle);
        state.mTimerHandle = fl::isr::handle();  // Reset to default (invalid) handle
