...
/// ## Usage Example
/// ```cpp
/// // ChannelEngineLcdRgb uses the IChannelDriver interface
/// CRGB leds[100];
/// void setup() {
///     FastLED.addLeds<WS2812, 1, GRB>(leds, 100);  // GPIO 1
///     // LCD RGB driver auto-selected on ESP32-P4
/// }
///
/// void loop() {
...
/// @brief Internal configuration structure for the LCD RGB channel driver
...
/// @brief LCD RGB-based channel driver for parallel LED control on ESP32-P4
///
/// Implements IChannelDriver interface using ESP32-P4 LCD RGB peripheral for
...
/// ```
class ChannelEngineLcdRgb : public IChannelDriver {
public:
    ...
    /// @return "LCD_RGB"
    fl::string getName() const override { return fl::string::from_literal("LCD_RGB"); }
...
/// @brief Factory function to create LCD RGB driver with real hardware peripheral
...
fl::shared_ptr<IChannelDriver> createLcdRgbEngine();
