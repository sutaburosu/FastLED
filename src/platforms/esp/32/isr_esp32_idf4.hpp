/*
  FastLED — ESP32 ISR Implementation (ESP-IDF 4.x)
  ------------------------------------------------
  ESP32-specific implementation of the cross-platform ISR API using legacy timer API.
  Supports ESP32, ESP32-S2, ESP32-S3 (Xtensa) and ESP32-C3 (RISC-V).

  This file is included by isr_esp32.hpp when ESP_IDF_VERSION < 5.0.0

  Note: This implementation uses the legacy timer API (driver/timer.h) which is
  deprecated in ESP-IDF 5.0+ but required for ESP-IDF 4.x compatibility.
...
// ESP32-S2/S3/C3 may have different configurations
...
struct esp32_idf4_isr_handle_data {
...
/**
 * Timer ISR callback - calls user handler
 * This runs in ISR context and must be IRAM-safe
 *
 * Note: ESP-IDF 4.x timer_isr_t returns bool indicating whether to yield
 */
static bool FL_IRAM timer_isr_callback_idf4(void* user_ctx)
{
    esp32_idf4_isr_handle_data* handle_data = static_cast<esp32_idf4_isr_handle_data*>(user_ctx);
 ...
 */
static void FL_IRAM gpio_isr_wrapper_idf4(void* arg)
{
    esp32_idf4_isr_handle_data* handle_data = static_cast<esp32_idf4_isr_handle_data*>(arg);
    if (handle_data && handle_data->user_handler) {
        handle_data->user_handler(handle_data->user_data);
    }
}
...
// =============================================================================
// ESP32 IDF4 ISR Implementation (fl::platform namespace)
// =============================================================================

inline int attach_timer_handler(const isr_config_t& config, isr_handle_t* out_handle) {
    if (!config.handler) {
        ESP_LOGW(ESP32_IDF4_ISR_TAG, "attachTimerHandler: handler is null");
        return -1;  // Invalid parameter
    }

    if (config.frequency_hz == 0) {
        ESP_LOGW(ESP32_IDF4_ISR_TAG, "attachTimerHandler: frequency_hz is 0");
        return -2;  // Invalid frequency
    }

    // Allocate handle data
    auto handle_owner = fl::make_unique<esp32_idf4_isr_handle_data>();
    auto* handle_data = handle_owner.get();
    if (!handle_data) {
        ESP_LOGW(ESP32_IDF4_ISR_TAG, "attachTimerHandler: failed to allocate handle data");
        return -3;  // Out of memory
    }

    // Allocate a timer
    if (!allocate_timer(&handle_data->timer_group, &handle_data->timer_idx)) {
        ESP_LOGW(ESP32_IDF4_ISR_TAG, "attachTimerHandler: no free timers available");
    ...
    if (alarm_value == 0) {
        ESP_LOGW(ESP32_IDF4_ISR_TAG, "attachTimerHandler: frequency too high (%lu Hz)",
    ...
    if (ret != ESP_OK) {
        ESP_LOGW(ESP32_IDF4_ISR_TAG, "attachTimerHandler: timer_init failed: %s", esp_err_to_name(ret));
        free_timer(handle_data->timer_group, handle_data->timer_idx);
    ...
    if (ret != ESP_OK) {
        ESP_LOGW(ESP32_IDF4_ISR_TAG, "attachTimerHandler: timer_set_counter_value failed: %s", esp_err_to_name(ret));
        timer_deinit(handle_data->timer_group, handle_data->timer_idx);
        free_timer(handle_data->timer_group, handle_data->timer_idx);
    ...
    if (ret != ESP_OK) {
        ESP_LOGW(ESP32_IDF4_ISR_TAG, "attachTimerHandler: timer_set_alarm_value failed: %s", esp_err_to_name(ret));
        timer_deinit(handle_data->timer_group, handle_data->timer_idx);
        free_timer(handle_data->timer_group, handle_data->timer_idx);
    ...
    if (ret != ESP_OK) {
        ESP_LOGW(ESP32_IDF4_ISR_TAG, "attachTimerHandler: timer_enable_intr failed: %s", esp_err_to_name(ret));
        timer_deinit(handle_data->timer_group, handle_data->timer_idx);
        free_timer(handle_data->timer_group, handle_data->timer_idx);
    ...
    ret = timer_isr_callback_add(handle_data->timer_group, handle_data->timer_idx,
                                  timer_isr_callback_idf4, handle_data, intr_flags);
    if (ret != ESP_OK) {
        ESP_LOGW(ESP32_IDF4_ISR_TAG, "attachTimerHandler: timer_isr_callback_add failed: %s", esp_err_to_name(ret));
    ...
    if (ret != ESP_OK) {
        ESP_LOGW(ESP32_IDF4_ISR_TAG, "attachTimerHandler: timer_start failed: %s", esp_err_to_name(ret));
        timer_isr_callback_remove(handle_data->timer_group, handle_data->timer_idx);
        timer_disable_intr(handle_data->timer_group, handle_data->timer_idx);
        timer_deinit(handle_data->timer_group, handle_data->timer_idx);
        free_timer(handle_data->timer_group, handle_data->timer_idx);
    ...
    ESP_LOGD(ESP32_IDF4_ISR_TAG, "Timer started: group=%d, idx=%d, freq=%lu Hz, alarm=%llu",
    ...
    if (out_handle) {
        ...
        out_handle->platform_id = ESP32_IDF4_PLATFORM_ID;
    }
...
inline int attach_external_handler(u8 pin, const isr_config_t& config, isr_handle_t* out_handle) {
    ...
    auto* handle_data = handle_owner.get();
    if (!handle_data) {
        ESP_LOGW(ESP32_IDF4_ISR_TAG, "attachExternalHandler: failed to allocate handle data");
        return -3;  // Out of memory
    }
    ...
    if (ret != ESP_OK) {
        ESP_LOGW(ESP32_IDF4_ISR_TAG, "attachExternalHandler: gpio_config failed: %s", esp_err_to_name(ret));
    ...
    if (!gpio_isr_service_installed) {
        taskENTER_CRITICAL(&gpio_isr_service_mutex_idf4);
        // Double-check after acquiring lock (classic double-checked locking pattern)
        if (!gpio_isr_service_installed) {
            ret = gpio_install_isr_service(0);
            if (ret != ESP_OK && ret != ESP_ERR_INVALID_STATE) {
                taskEXIT_CRITICAL(&gpio_isr_service_mutex_idf4);
                ESP_LOGW(ESP32_IDF4_ISR_TAG, "attachExternalHandler: gpio_install_isr_service failed: %s", esp_err_to_name(ret));
    ...
    ret = gpio_isr_handler_add(static_cast<gpio_num_t>(pin), gpio_isr_wrapper_idf4, handle_data);
    if (ret != ESP_OK) {
        ESP_LOGW(ESP32_IDF4_ISR_TAG, "attachExternalHandler: gpio_isr_handler_add failed: %s", esp_err_to_name(ret));
    ...
    ESP_LOGD(ESP32_IDF4_ISR_TAG, "GPIO interrupt attached on pin %d", pin);
...
inline int detach_handler(isr_handle_t& handle) {
    if (!handle.is_valid() || handle.platform_id != ESP32_IDF4_PLATFORM_ID) {
        ESP_LOGW(ESP32_IDF4_ISR_TAG, "detachHandler: invalid handle");
        return -1;  // Invalid handle
    }

    esp32_idf4_isr_handle_data* handle_data = static_cast<esp32_idf4_isr_handle_data*>(handle.platform_handle);
    if (!handle_data) {
        ESP_LOGW(ESP32_IDF4_ISR_TAG, "detachHandler: null handle data");
        return -1;  // Invalid handle
    }
    ...
    ESP_LOGD(ESP32_IDF4_ISR_TAG, "Handler detached");
    return 0;  // Success
}

inline int enable_handler(const isr_handle_t& handle) {
    if (!handle.is_valid() || handle.platform_id != ESP32_IDF4_PLATFORM_ID) {
        ESP_LOGW(ESP32_IDF4_ISR_TAG, "enableHandler: invalid handle");
        return -1;  // Invalid handle
    }

    esp32_idf4_isr_handle_data* handle_data = static_cast<esp32_idf4_isr_handle_data*>(handle.platform_handle);
    if (!handle_data) {
        ESP_LOGW(ESP32_IDF4_ISR_TAG, "enableHandler: null handle data");
        return -1;  // Invalid handle
    }
    ...
    if (handle_data->is_timer) {
        ...
        if (ret != ESP_OK) {
            ESP_LOGW(ESP32_IDF4_ISR_TAG, "enableHandler: timer_start failed: %s", esp_err_to_name(ret));
        ...
        if (ret != ESP_OK) {
            ESP_LOGW(ESP32_IDF4_ISR_TAG, "enableHandler: gpio_intr_enable failed: %s", esp_err_to_name(ret));
            return -14;
        }
        handle_data->is_enabled = true;
    }
...
inline int disable_handler(const isr_handle_t& handle) {
    if (!handle.is_valid() || handle.platform_id != ESP32_IDF4_PLATFORM_ID) {
        ESP_LOGW(ESP32_IDF4_ISR_TAG, "disableHandler: invalid handle");
        return -1;  // Invalid handle
    }

    esp32_idf4_isr_handle_data* handle_data = static_cast<esp32_idf4_isr_handle_data*>(handle.platform_handle);
    if (!handle_data) {
        ESP_LOGW(ESP32_IDF4_ISR_TAG, "disableHandler: null handle data");
        return -1;  // Invalid handle
    }
    ...
    if (handle_data->is_timer) {
        ...
        if (ret != ESP_OK) {
            ESP_LOGW(ESP32_IDF4_ISR_TAG, "disableHandler: timer_pause failed: %s", esp_err_to_name(ret));
        ...
        if (ret != ESP_OK) {
            ESP_LOGW(ESP32_IDF4_ISR_TAG, "disableHandler: gpio_intr_disable failed: %s", esp_err_to_name(ret));
            return -15;
        }
        handle_data->is_enabled = false;
    }
