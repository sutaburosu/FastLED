...
public:
    ...
    virtual u8* getFrameBuffer() = 0;

    /// @brief Get size of the frame buffer in bytes
    virtual u32 getFrameBufferSize() const = 0;
...
public:
    ...
    virtual ObjectFLEDPinResult validatePin(u8 pin) const = 0;
    ...
    virtual fl::unique_ptr<IObjectFLEDInstance> createInstance(
        ...
        u32 t1_ns, u32 t2_ns, u32 t3_ns, u32 reset_us) = 0;

    /// @brief Get the platform-specific peripheral instance
    static fl::shared_ptr<IObjectFLEDPeripheral> create();
