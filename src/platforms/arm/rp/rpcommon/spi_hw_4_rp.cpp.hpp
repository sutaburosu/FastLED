...
bool SPIQuadRP2040::transmit(TransmitMode mode) {
    ...
    for (int i = (int)word_count - 1; i >= 0; i--) {
        ...
        for (int bit = 0; bit < 8; bit++) {
            ...
            u32 bit0 = (byte0 >> (7 - bit)) & 1;
            u32 bit1 = (byte1 >> (7 - bit)) & 1;
            u32 bit2 = (byte2 >> (7 - bit)) & 1;
            u32 bit3 = (byte3 >> (7 - bit)) & 1;

            // Pack 4 bits into a nibble, MSB first
            u32 nibble = (bit0 << 3) | (bit1 << 2) | (bit2 << 1) | bit3;
...
bool SPIQuadRP2040::waitComplete(u32 timeout_ms) {
    if (!mTransactionActive) {
