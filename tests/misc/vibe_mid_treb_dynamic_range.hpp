// Repro test for audio::detector::Vibe mid/treble stuck at 1.00
...
audio::Sample makeMix(float bassAmp, float midAmp, float trebAmp,
...
// Generate a multi-frequency audio::Sample with broadband noise floor.
// Models a MEMS mic (INMP441): self-noise adds continuous energy to all
// audio::fft::FFT bins. noiseAmp is the peak amplitude of the white noise.
// Uses deterministic PRNG seeded from timestamp for reproducibility.
audio::Sample makeMixWithNoise(float bassAmp, float midAmp, float trebAmp,
...
// content plus noise. The 512-sample audio::fft::FFT window causes the 3500 Hz tone
// to leak significant energy into the mid bin.
audio::Sample makeMixWithLeakage(float bassAmp, float leakageBassAmp,
    ...
    // audio::fft::FFT spectral leakage bleeds energy into the mid bin (3688-7356 Hz).
...
FL_TEST_CASE("audio::detector::Vibe mid dynamic range with alternating signal") {
    ...
    for (int i = 0; i < 100; ++i) {
        ...
        auto ctx = make_shared<audio::Context>(sample);
...
FL_TEST_CASE("audio::detector::Vibe treble dynamic range with alternating signal") {
    audio::detector::Vibe detector;
...
FL_TEST_CASE("audio::detector::Vibe mid dynamic range with gradual variation (realistic)") {
    audio::detector::Vibe detector;
...
FL_TEST_CASE("audio::detector::Vibe mid with sustained signal and brief gaps") {
    audio::detector::Vibe detector;
...
FL_TEST_CASE("audio::detector::Vibe bass dynamic range with alternating signal (control)") {
    audio::detector::Vibe detector;
...
FL_TEST_CASE("audio::detector::Vibe mid dynamic range with MEMS noise floor") {
    audio::detector::Vibe detector;
...
FL_TEST_CASE("audio::detector::Vibe treble dynamic range with MEMS noise floor") {
    audio::detector::Vibe detector;
...
FL_TEST_CASE("audio::detector::Vibe mid dynamic range with spectral leakage") {
    audio::detector::Vibe detector;
    const float noiseAmp = 150.0f;
...
FL_TEST_CASE("audio::detector::Vibe treble worst case: noise + leakage + gradual") {
    audio::detector::Vibe detector;
    ...
    for (int frame = 0; frame < 300; ++frame) {
        ...
        auto sample = audio::Sample(data, (300 + frame) * 12);
        auto ctx = make_shared<audio::Context>(sample);
        ctx->setSampleRate(44100);
        detector.update(ctx);
