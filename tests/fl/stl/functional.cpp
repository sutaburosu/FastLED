...
FL_TEST_CASE("fl::invoke with free functions") {
    FL_SUBCASE("function with arguments") {
        int result = invoke(functional_test::add, 3, 4);
        FL_CHECK_EQ(result, 7);
    }

    FL_SUBCASE("function with no arguments") {
        int result = invoke(functional_test::get_42);
        FL_CHECK_EQ(result, 42);
    }

    FL_SUBCASE("function pointer") {
        int (*fn_ptr)(int, int) = &functional_test::add;
        int result = invoke(fn_ptr, 10, 20);
        FL_CHECK_EQ(result, 30);
    }
}

// Test invoke with member function pointers
FL_TEST_CASE("fl::invoke with member function pointers") {
    FL_SUBCASE("member function with object reference") {
        Calculator calc(5);
        int result = invoke(&Calculator::add, calc, 3);
        FL_CHECK_EQ(result, 8);
    }

    FL_SUBCASE("const member function with object reference") {
        const Calculator calc(10);
        int result = invoke(&Calculator::add_const, calc, 5);
        FL_CHECK_EQ(result, 15);
    }

    FL_SUBCASE("member function with multiple arguments") {
        Calculator calc(0);
        int result = invoke(&Calculator::multiply, calc, 6, 7);
        FL_CHECK_EQ(result, 42);
    }

    FL_SUBCASE("member function with pointer") {
        Calculator calc(20);
        Calculator* ptr = &calc;
    ...
    FL_SUBCASE("member function with const pointer") {
        const Calculator calc(15);
        const Calculator* ptr = &calc;
        int result = invoke(&Calculator::add_const, ptr, 5);
        FL_CHECK_EQ(result, 20);
    }
}

// Test invoke with member data pointers
FL_TEST_CASE("fl::invoke with member data pointers") {
    FL_SUBCASE("member data with object reference") {
        Calculator calc(99);
        int result = invoke(&Calculator::value, calc);
        FL_CHECK_EQ(result, 99);
    }

    FL_SUBCASE("member data with pointer") {
        Calculator calc(42);
        Calculator* ptr = &calc;
    ...
    FL_SUBCASE("member data with const pointer") {
        const Calculator calc(100);
        const Calculator* ptr = &calc;
        int result = invoke(&Calculator::value, ptr);
        FL_CHECK_EQ(result, 100);
    }
}

// Test invoke with functors
FL_TEST_CASE("fl::invoke with functors") {
    FL_SUBCASE("functor object") {
        Multiplier mult(3);
        int result = invoke(mult, 7);
        FL_CHECK_EQ(result, 21);
    }

    FL_SUBCASE("functor with move") {
        Multiplier mult(5);
        int result = invoke(fl::move(mult), 8);
        FL_CHECK_EQ(result, 40);
    }
}

// Test invoke with lambdas
FL_TEST_CASE("fl::invoke with lambdas") {
    FL_SUBCASE("lambda with no capture") {
        auto lambda = [](int x, int y) { return x + y; };
        int result = invoke(lambda, 3, 4);
        FL_CHECK_EQ(result, 7);
    }

    FL_SUBCASE("lambda with capture") {
        int factor = 10;
    ...
    FL_SUBCASE("lambda with mutable capture") {
        int counter = 0;
        auto lambda = [&counter]() { return ++counter; };
        int result1 = invoke(lambda);
        int result2 = invoke(lambda);
        FL_CHECK_EQ(result1, 1);
        FL_CHECK_EQ(result2, 2);
    }

    FL_SUBCASE("lambda returning void") {
        int value = 0;
...
FL_TEST_CASE("fl::invoke with static member functions") {
    FL_SUBCASE("static member function") {
...
FL_TEST_CASE("fl::invoke helper traits") {
...
// Test edge cases
FL_TEST_CASE("fl::invoke edge cases") {
    FL_SUBCASE("invoke with temporary object") {
        int result = invoke(&Calculator::add, Calculator(100), 50);
        FL_CHECK_EQ(result, 150);
    }

    FL_SUBCASE("invoke with temporary functor") {
        int result = invoke(Multiplier(4), 10);
        FL_CHECK_EQ(result, 40);
    }

    FL_SUBCASE("invoke with temporary lambda") {
        int result = invoke([](int x) { return x * x; }, 9);
        FL_CHECK_EQ(result, 81);
    }

    FL_SUBCASE("nested invoke calls") {
        ...
        FL_CHECK_EQ(product, 14);
    }
}

// Test invoke with different return types
FL_TEST_CASE("fl::invoke with various return types") {
    FL_SUBCASE("return bool") {
        auto is_positive = [](int x) { return x > 0; };
        bool result = invoke(is_positive, 5);
        FL_CHECK(result);
    }

    FL_SUBCASE("return float") {
        auto divide = [](float a, float b) { return a / b; };
        float result = invoke(divide, 10.0f, 2.0f);
        FL_CHECK_EQ(result, 5.0f);
    }

    FL_SUBCASE("return void") {
        int side_effect = 0;
        auto set_value = [&side_effect](int x) { side_effect = x; };
        invoke(set_value, 99);
        FL_CHECK_EQ(side_effect, 99);
    }
}

// Test invoke forwarding behavior
FL_TEST_CASE("fl::invoke forwarding") {
    FL_SUBCASE("forwards arguments correctly") {
        struct MoveOnly {
            int value;
            bool moved_from;
        ...
        FL_CHECK_EQ(result, 42);
        FL_CHECK(obj.moved_from); // Verify object was moved from
    }
}

// Test fl::invoke with fl::scoped_ptr smart pointers
// (Merged from test_invoke.cpp)
FL_TEST_CASE("fl::invoke with scoped_ptr smart pointers") {
    ...
    FL_CHECK_EQ(123, scopedPtr->value);

    // Member function with additional arg, const
    FL_CHECK_EQ(133, invoke(&TestScopedPtrClass::add, scopedPtr, 10));

    // Member function with additional arg, non-const
    FL_CHECK_EQ(246, invoke(&TestScopedPtrClass::multiply, scopedPtr, 2));

    // Member data pointer access and modification
    FL_CHECK_EQ(123, invoke(&TestScopedPtrClass::value, scopedPtr));
    invoke(&TestScopedPtrClass::value, scopedPtr) = 999;
    FL_CHECK_EQ(999, scopedPtr->value);
    ...
    FL_CHECK_EQ(555, customScopedPtr->value);
    FL_CHECK_EQ(565, invoke(&TestScopedPtrClass::add, customScopedPtr, 10));
}

// Test fl::invoke with fl::function objects
// (Merged from test_invoke.cpp)
FL_TEST_CASE("fl::invoke with fl::function objects") {
    ...
    FL_CHECK_EQ(900, result6);  // 300 * 3 = 900
