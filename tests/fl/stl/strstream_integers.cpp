
#include "fl/stl/strstream.h"
#include "fl/stl/stdint.h"
#include "fl/stl/new.h"
#include "test.h"
#include "fl/stl/int.h"
#include "fl/stl/move.h"
#include "fl/stl/string.h"

FL_TEST_FILE(FL_FILEPATH) {

FL_TEST_CASE("sstream handles all integer types generically") {
    fl::sstream ss;

    FL_SUBCASE("signed char types") {
        signed char sc = -42;
        ss << sc;
        FL_CHECK(ss.str() == "-42");
    }

    FL_SUBCASE("unsigned char types") {
        unsigned char uc = 200;
        ss.clear();
        ss << uc;
        FL_CHECK(ss.str() == "200");
    }

    FL_SUBCASE("short types") {
        short s = -12345;
        ss.clear();
        ss << s;
        FL_CHECK(ss.str() == "-12345");

        unsigned short us = 54321;
        ss.clear();
        ss << us;
        FL_CHECK(ss.str() == "54321");
    }

    FL_SUBCASE("int types") {
        int i = -123456;
        ss.clear();
        ss << i;
        FL_CHECK(ss.str() == "-123456");

        unsigned int ui = 654321;
        ss.clear();
        ss << ui;
        FL_CHECK(ss.str() == "654321");
    }

    FL_SUBCASE("long types") {
        long l = -1234567;
        ss.clear();
        ss << l;
        FL_CHECK(ss.str() == "-1234567");

        unsigned long ul = 7654321;
        ss.clear();
        ss << ul;
        FL_CHECK(ss.str() == "7654321");
    }

    FL_SUBCASE("long long types") {
        long long ll = -123456789012345LL;
        ss.clear();
        ss << ll;
        FL_CHECK(ss.str() == "-123456789012345");

        unsigned long long ull = 987654321098765ULL;
        ss.clear();
        ss << ull;
        FL_CHECK(ss.str() == "987654321098765");
    }

    FL_SUBCASE("fixed-width integer types") {
        int8_t i8 = -127;
        ss.clear();
        ss << i8;
        FL_CHECK(ss.str() == "-127");

        uint8_t u8 = 255;
        ss.clear();
        ss << u8;
        FL_CHECK(ss.str() == "255");

        int16_t i16 = -32000;
        ss.clear();
        ss << i16;
        FL_CHECK(ss.str() == "-32000");

        uint16_t u16 = 65000;
        ss.clear();
        ss << u16;
        FL_CHECK(ss.str() == "65000");

        int32_t i32 = -2000000000;
        ss.clear();
        ss << i32;
        FL_CHECK(ss.str() == "-2000000000");

        uint32_t u32 = 4000000000U;
        ss.clear();
        ss << u32;
        FL_CHECK(ss.str() == "4000000000");

        int64_t i64 = -9000000000000000LL;
        ss.clear();
        ss << i64;
        FL_CHECK(ss.str() == "-9000000000000000");

        uint64_t u64 = 18000000000000000ULL;
        ss.clear();
        ss << u64;
        FL_CHECK(ss.str() == "18000000000000000");
    }

    FL_SUBCASE("fl:: integer types") {
        fl::i8 fi8 = -100;
        ss.clear();
        ss << fi8;
        FL_CHECK(ss.str() == "-100");

        fl::u16 fu16 = 50000;
        ss.clear();
        ss << fu16;
        FL_CHECK(ss.str() == "50000");

        fl::i32 fi32 = -1000000;
        ss.clear();
        ss << fi32;
        FL_CHECK(ss.str() == "-1000000");

        fl::u32 fu32 = 3000000000U;
        ss.clear();
        ss << fu32;
        FL_CHECK(ss.str() == "3000000000");

        fl::i64 fi64 = -5000000000000LL;
        ss.clear();
        ss << fi64;
        FL_CHECK(ss.str() == "-5000000000000");

        fl::u64 fu64 = 10000000000000ULL;
        ss.clear();
        ss << fu64;
        FL_CHECK(ss.str() == "10000000000000");
    }

    FL_SUBCASE("mixed integer types in single stream") {
        ss.clear();
        ss << int16_t(-100) << " " << uint32_t(123456) << " " << int64_t(-999999999);
        FL_CHECK(ss.str() == "-100 123456 -999999999");
    }
}

FL_TEST_CASE("StrN write handles all integer types generically") {
    fl::StrN<64> sn;

    FL_SUBCASE("various integer types") {
        int16_t i16 = -1000;
        sn.write(i16);
        FL_CHECK(fl::string(sn.c_str()) == "-1000");

        sn.clear();
        uint32_t u32 = 2000000000U;
        sn.write(u32);
        FL_CHECK(fl::string(sn.c_str()) == "2000000000");

        sn.clear();
        int64_t i64 = -3000000000000LL;
        sn.write(i64);
        FL_CHECK(fl::string(sn.c_str()) == "-3000000000000");
    }
}

FL_TEST_CASE("string append handles all integer types generically") {
    fl::string s;

    FL_SUBCASE("various integer types") {
        s.append(int16_t(-5000));
        FL_CHECK(s == "-5000");

        s.clear();
        s.append(uint32_t(4000000000U));
        FL_CHECK(s == "4000000000");

        s.clear();
        s.append(int64_t(-7000000000000LL));
        FL_CHECK(s == "-7000000000000");

        s.clear();
        s.append((long)123456);
        FL_CHECK(s == "123456");

        s.clear();
        s.append((unsigned long)987654);
        FL_CHECK(s == "987654");
    }

    FL_SUBCASE("mixed integer types") {
        s.clear();
        s.append((short)-100);
        s.append(" ");
        s.append((unsigned int)200000);
        s.append(" ");
        s.append((long long)-300000000000LL);
        FL_CHECK(s == "-100 200000 -300000000000");
    }
}

FL_TEST_CASE("char types are handled correctly") {
    fl::sstream ss;

    FL_SUBCASE("char is treated as character by default") {
        char c = 'A';
        ss << c;
        FL_CHECK(ss.str() == "A");
    }

    FL_SUBCASE("char can be treated as integer with flag") {
        ss.setTreatCharAsInt(true);
        char c = 65;  // ASCII 'A'
        ss << c;
        FL_CHECK(ss.str() == "65");
    }

    FL_SUBCASE("signed char is treated as number") {
        signed char sc = 65;
        ss << sc;
        FL_CHECK(ss.str() == "65");
    }

    FL_SUBCASE("unsigned char is treated as number") {
        unsigned char uc = 65;
        ss << uc;
        FL_CHECK(ss.str() == "65");
    }
}

} // FL_TEST_FILE
