#include "fl/net/http/http_parser.h"
...
static fl::span<const uint8_t> asSpan(const char* s) {
    return fl::span<const uint8_t>(
        reinterpret_cast<const uint8_t*>(s), fl::strlen(s)); // ok reinterpret cast
}
...
FL_TEST_CASE("HttpRequestParser - Simple GET request") {
    ...
    FL_CHECK(req->headers.at("Host") == "localhost");
...
FL_TEST_CASE("HttpRequestParser - POST with Content-Length") {
    ...
    FL_CHECK(req->headers.at("Content-Type") == "application/json");
    FL_CHECK(req->headers.at("Content-Length") == "13");
...
FL_TEST_CASE("HttpRequestParser - POST with chunked encoding") {
    ...
    FL_CHECK(req->method == "POST");
    FL_CHECK(req->uri == "/rpc");
    FL_CHECK(req->headers.at("Transfer-Encoding") == "chunked");
    FL_CHECK(req->body.size() == 13);
...
FL_TEST_CASE("HttpRequestParser - Incremental parsing") {
    ...
    parser.feed(asSpan(part1));
    FL_CHECK_FALSE(parser.isComplete());

    parser.feed(asSpan(part2));
    FL_CHECK_FALSE(parser.isComplete());

    parser.feed(asSpan(part3));
    FL_CHECK_FALSE(parser.isComplete());

    parser.feed(asSpan(part4));
    FL_CHECK_FALSE(parser.isComplete());

    parser.feed(asSpan(part5));
    FL_CHECK(parser.isComplete());
...
FL_TEST_CASE("HttpRequestParser - Multiple headers with same name") {
    ...
    FL_CHECK(req->headers.at("Accept") == "application/json");
}
...
FL_TEST_CASE("HttpResponseParser - Response with chunked encoding") {
    ...
    FL_REQUIRE(res);
    FL_CHECK(res->statusCode == 200);
    FL_CHECK(res->headers.at("Transfer-Encoding") == "chunked");
...
FL_TEST_CASE("HttpRequestParser - Header value trimming") {
    ...
    FL_REQUIRE(req);
    FL_CHECK(req->headers.at("Host") == "localhost");
    FL_CHECK(req->headers.at("Accept") == "text/html");
}
