...
protected:
    ...
    int sendData(fl::span<const uint8_t> data) override {
        ...
        for (auto& pair : mClientQueues) {
            pair.second.sentData.insert(pair.second.sentData.end(), data.begin(), data.end());
        }
    ...
    int recvData(fl::span<uint8_t> buffer) override {
        ...
        for (auto& pair : mClientQueues) {
            ...
            if (!recvQueue.empty()) {
                size_t toCopy = (recvQueue.size() < buffer.size()) ? recvQueue.size() : buffer.size();
                fl::memcpy(buffer.data(), recvQueue.data(), toCopy);
