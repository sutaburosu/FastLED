...
FL_TEST_CASE("Corkscrew Circle10 test") {
    ...
    fl::Corkscrew corkscrew_simple(1.0f, 10);
    FL_REQUIRE_EQ(corkscrew_simple.cylinderWidth(), 10);   // ceil(10 LEDs / 1 turn) = 10
    FL_REQUIRE_EQ(corkscrew_simple.cylinderHeight(), 1);   // ceil(1 turn) = 1
    
    // Test 2 turns with 20 LEDs (10 LEDs per turn)
    fl::Corkscrew corkscrew_example(2.0f, 20);
    FL_REQUIRE_EQ(corkscrew_example.cylinderWidth(), 10);  // LEDs per turn
    FL_REQUIRE_EQ(corkscrew_example.cylinderHeight(), 2);  // Number of turns
    
    // Test default case: 19 turns, 144 LEDs
    fl::Corkscrew corkscrew_default(19.0f, 144);
    FL_REQUIRE_EQ(corkscrew_default.cylinderWidth(), 8);   // ceil(144/19) = ceil(7.58) = 8
    FL_REQUIRE_EQ(corkscrew_default.cylinderHeight(), 18); // Optimized: ceil(144/8) = ceil(18) = 18
    
    // Test FestivalStick case: 19 turns, 288 LEDs 
    fl::Corkscrew corkscrew_festival(19.0f, 288);
    FL_REQUIRE_EQ(corkscrew_festival.cylinderWidth(), 16);  // ceil(288/19) = ceil(15.16) = 16  
    FL_REQUIRE_EQ(corkscrew_festival.cylinderHeight(), 18); // ceil(288/16) = ceil(18) = 18 (optimized!)
    
    // Verify grid size matches LED count
    FL_REQUIRE_EQ(corkscrew_festival.cylinderWidth() * corkscrew_festival.cylinderHeight(), 288);
    ...
    for (uint16_t i = 0; i < corkscrew_festival.size(); ++i) {
        fl::vec2f pos = corkscrew_festival.at_no_wrap(i);
        max_height = fl::max(max_height, pos.y);
        min_height = fl::min(min_height, pos.y);
    }
    
    // LEDs should span from 0 to height-1
    FL_REQUIRE(min_height >= 0.0f);
...
FL_TEST_CASE("Corkscrew LED distribution test") {
    ...
    for (uint16_t i = 0; i < corkscrew.size(); ++i) {
        fl::vec2f pos = corkscrew.at_no_wrap(i);
    ...
    FL_REQUIRE(row_counts[corkscrew.cylinderHeight() - 1] > 0); // Top row should have LEDs
...
FL_TEST_CASE("Corkscrew two turns test") {
    ...
    FL_REQUIRE_EQ(corkscrew_two_turns.cylinderWidth(), 2);   // LEDs per turn
    FL_REQUIRE_EQ(corkscrew_two_turns.cylinderHeight(), 2);  // Number of turns
    
    // Verify grid size matches LED count
    FL_REQUIRE_EQ(corkscrew_two_turns.cylinderWidth() * corkscrew_two_turns.cylinderHeight(), 4);
    
    // Test LED positioning across both turns
    FL_REQUIRE_EQ(corkscrew_two_turns.size(), 4);
    ...
    fl::vec2f pos0 = corkscrew_two_turns.at_no_wrap(0);
    fl::vec2f pos1 = corkscrew_two_turns.at_no_wrap(1);
    fl::vec2f pos2 = corkscrew_two_turns.at_no_wrap(2);
    fl::vec2f pos3 = corkscrew_two_turns.at_no_wrap(3);
    ...
    FL_REQUIRE(row_counts[0] > 0);  // First turn should have LEDs
...
FL_TEST_CASE("Constexpr corkscrew dimension calculation") {
    ...
    FL_REQUIRE_EQ(festival_width, runtime_corkscrew.cylinderWidth());
    FL_REQUIRE_EQ(festival_height, runtime_corkscrew.cylinderHeight());
...
FL_TEST_CASE("TestCorkscrewBufferFunctionality") {
    ...
    FL_REQUIRE(surface.size() == width * height);
    ...
    FL_REQUIRE(found_blue);
    FL_REQUIRE(found_green);
...
FL_TEST_CASE("Corkscrew readFrom with bilinear interpolation") {
    ...
    fl::span<CRGB> led_span(led_buffer);
    fl::Corkscrew corkscrew(1.0f, led_span, false);
    ...
    // Verify LED count
    FL_REQUIRE_EQ(corkscrew.size(), 12);
    ...
    FL_REQUIRE(non_black_count > 0);
    FL_REQUIRE(found_red_component);
    ...
    fl::vec2f pos0 = corkscrew.at_no_wrap(0);
    fl::vec2f pos5 = corkscrew.at_no_wrap(5);
    
    // Positions should be different
    bool positions_different = (pos0.x != pos5.x) || (pos0.y != pos5.y);
    FL_REQUIRE(positions_different);
    
    // Positions should be within reasonable bounds
    FL_REQUIRE(pos0.x >= 0.0f);
    FL_REQUIRE(pos0.y >= 0.0f);
    FL_REQUIRE(pos5.x >= 0.0f);
...
FL_TEST_CASE("Corkscrew span data access") {
    ...
    for (size_t i = 0; i < expected_size; ++i) {
        FL_REQUIRE_EQ(data_ptr[i].r, 0);
        FL_REQUIRE_EQ(data_ptr[i].g, 0);
        FL_REQUIRE_EQ(data_ptr[i].b, 0);
    }
    ...
    if (expected_size > 0) {
        ...
        FL_REQUIRE_EQ(data_ptr[0].r, 255);
        FL_REQUIRE_EQ(data_ptr[0].g, 0);
        FL_REQUIRE_EQ(data_ptr[0].b, 0);
...
FL_TEST_CASE("Corkscrew ScreenMap functionality") {
    ...
    FL_REQUIRE_EQ(screenMap.getLength(), 8);
    
    // Verify default diameter
    FL_REQUIRE_EQ(screenMap.getDiameter(), 0.5f);
    
    // Test custom diameter
    fl::ScreenMap screenMapCustom = corkscrew.toScreenMap(1.2f);
    FL_REQUIRE_EQ(screenMapCustom.getDiameter(), 1.2f);
    
    // Verify that each LED index maps to the same position as at_exact() (wrapped)
    for (uint16_t i = 0; i < 8; ++i) {
        fl::vec2f corkscrewPos = corkscrew.at_exact(i);
        fl::vec2f screenMapPos = screenMap[i];
        
        // Positions should match exactly (both are wrapped)
        FL_REQUIRE(fl::almost_equal(corkscrewPos.x, screenMapPos.x));
        FL_REQUIRE(fl::almost_equal(corkscrewPos.y, screenMapPos.y));
    }
    
    // Test that different LED indices have different positions (at least some of them)
    bool positions_differ = false;
    for (uint16_t i = 1; i < 8; ++i) {
        fl::vec2f pos0 = screenMap[0];
        fl::vec2f posI = screenMap[i];
        if (!fl::almost_equal(pos0.x, posI.x) || !fl::almost_equal(pos0.y, posI.y)) {
            positions_differ = true;
            break;
        }
    }
    FL_REQUIRE(positions_differ); // At least some positions should be different
    
    // Test ScreenMap bounds
    fl::vec2f bounds = screenMap.getBounds();
    FL_REQUIRE(bounds.x > 0.0f); // Should have some width
    FL_REQUIRE(bounds.y >= 0.0f); // Should have some height (or 0 for single row)
    
    // Test a larger corkscrew to ensure it works with more complex cases
    fl::Corkscrew corkscrew_large(19.0f, 288, false, fl::Gap()); // FestivalStick case
    fl::ScreenMap screenMap_large = corkscrew_large.toScreenMap(0.8f);
    
    FL_REQUIRE_EQ(screenMap_large.getLength(), 288);
    FL_REQUIRE_EQ(screenMap_large.getDiameter(), 0.8f);
    
    // Verify all positions are valid (non-negative)
    for (uint16_t i = 0; i < 288; ++i) {
        fl::vec2f pos = screenMap_large[i];
        FL_REQUIRE(pos.x >= 0.0f);
        FL_REQUIRE(pos.y >= 0.0f);
    }
}
...
FL_TEST_CASE("Corkscrew Gap struct functionality") {
    ...
    fl::Gap halfGap(0.5f);
    FL_REQUIRE_EQ(halfGap.gap, 0.5f);
    
    fl::Gap fullGap(1.0f);
    FL_REQUIRE_EQ(fullGap.gap, 1.0f);
    
    // Test Corkscrew construction with Gap struct
    fl::Gap customGap(0.3f);
    fl::Corkscrew corkscrewWithGap(19.0f, 144, false, customGap);
    FL_REQUIRE_EQ(corkscrewWithGap.size(), 144);
    
    // Test that different gap values still produce valid corkscrews
    fl::Gap noGap(0.0f);
    fl::Gap smallGap(0.1f);
    fl::Gap largeGap(0.9f);
    
    fl::Corkscrew corkscrewNoGap(2.0f, 8, false, noGap);
    fl::Corkscrew corkscrewSmallGap(2.0f, 8, false, smallGap);
    fl::Corkscrew corkscrewLargeGap(2.0f, 8, false, largeGap);
    
    // All should have the same basic properties since gap computation is not implemented yet
    FL_REQUIRE_EQ(corkscrewNoGap.size(), 8);
    FL_REQUIRE_EQ(corkscrewSmallGap.size(), 8);
    FL_REQUIRE_EQ(corkscrewLargeGap.size(), 8);
    
    // Test that Gap struct supports copy construction and assignment
    fl::Gap originalGap(0.7f);
    fl::Gap copiedGap(originalGap);
    FL_REQUIRE_EQ(copiedGap.gap, 0.7f);
    
    fl::Gap assignedGap;
    assignedGap = originalGap;
    FL_REQUIRE_EQ(assignedGap.gap, 0.7f);
    
    // Test Gap struct in corkscrew construction
    fl::Corkscrew corkscrewWithGap2(19.0f, 144, false, customGap);
    FL_REQUIRE(corkscrewWithGap2.cylinderWidth() > 0);
...
FL_TEST_CASE("Corkscrew Enhanced Gap - Specific user test: 2 LEDs, 1 turn, 1.0f gap every 1 LED") {
    ...
    FL_REQUIRE_EQ(1.0f, 1.0f);
    
    // Get positions for both LEDs (unwrapped and wrapped)
    fl::vec2f pos0_unwrapped = corkscrew.at_no_wrap(0); // First LED, no gap yet
    fl::vec2f pos1_unwrapped = corkscrew.at_no_wrap(1); // Second LED, after gap trigger
    fl::vec2f pos0_wrapped = corkscrew.at_exact(0); // First LED, wrapped
    fl::vec2f pos1_wrapped = corkscrew.at_exact(1); // Second LED, wrapped
    ...
    FL_REQUIRE(pos0_unwrapped.x >= 0.0f);
    FL_REQUIRE(pos0_unwrapped.y >= 0.0f);
    FL_REQUIRE(pos1_unwrapped.x >= 0.0f);
    FL_REQUIRE(pos1_unwrapped.y >= 0.0f);
    
    // Both wrapped positions should be valid
    FL_REQUIRE(pos0_wrapped.x >= 0.0f);
    FL_REQUIRE(pos0_wrapped.y >= 0.0f);
    FL_REQUIRE(pos1_wrapped.x >= 0.0f);
    FL_REQUIRE(pos1_wrapped.y >= 0.0f);
    
    // First LED should be at or near starting position
    FL_REQUIRE(pos0_unwrapped.x <= static_cast<float>(width));
    FL_REQUIRE(pos0_unwrapped.y <= 1.0f);
    
    // Wrapped positions should be within the cylinder width
    FL_REQUIRE(pos0_wrapped.x < static_cast<float>(width));
    FL_REQUIRE(pos1_wrapped.x < static_cast<float>(width));
    
    // The key test: total height should not exceed the specified turns
    float maxHeight = fl::max(pos0_unwrapped.y, pos1_unwrapped.y);
    FL_REQUIRE(maxHeight <= 1.0f + 0.1f); // Small tolerance for floating point
    ...
    FL_REQUIRE(unwrapped_different);
...
FL_TEST_CASE("Corkscrew gap test with 3 LEDs") {
    ...
    fl::Gap gapParams(1, 0.5f); // gap of 0.5 every 1 LED
    
    fl::Corkscrew corkscrew_gap(1.0f, 3, false, gapParams);
    FL_REQUIRE_EQ(corkscrew_gap.size(), 3);
    
    // Get LED positions
    fl::vec2f pos0 = corkscrew_gap.at_exact(0);  // wrapped position
    fl::vec2f pos1 = corkscrew_gap.at_exact(1);  // wrapped position
    fl::vec2f pos2 = corkscrew_gap.at_exact(2);  // wrapped position
    ...
    fl::vec2f pos0_unwrap = corkscrew_gap.at_no_wrap(0);
    fl::vec2f pos1_unwrap = corkscrew_gap.at_no_wrap(1);
    fl::vec2f pos2_unwrap = corkscrew_gap.at_no_wrap(2);
    ...
    FL_REQUIRE(fl::almost_equal(pos0_unwrap.x, 0.0f)); // LED0 at unwrapped w=0
    FL_REQUIRE(fl::almost_equal(pos1_unwrap.x, 3.0f)); // LED1 at unwrapped w=3.0 ✓
    FL_REQUIRE(fl::almost_equal(pos2_unwrap.x, 6.0f)); // LED2 at unwrapped w=6.0
    
    // Test wrapped positions - all LEDs wrap back to w=0 due to width=3
    FL_REQUIRE(fl::almost_equal(pos0.x, 0.0f)); // LED0 wrapped w=0
    FL_REQUIRE(fl::almost_equal(pos1.x, 0.0f)); // LED1 wrapped w=0 (3.0 % 3 = 0) 
    FL_REQUIRE(fl::almost_equal(pos2.x, 0.0f)); // LED2 wrapped w=0 (6.0 % 3 = 0)
    
    // Height positions should increase by 1 for each LED (one turn per 3 width units)
    FL_REQUIRE(fl::almost_equal(pos0_unwrap.y, 0.0f)); // LED0 height
    FL_REQUIRE(fl::almost_equal(pos1_unwrap.y, 1.0f)); // LED1 height  
    FL_REQUIRE(fl::almost_equal(pos2_unwrap.y, 2.0f)); // LED2 height
}
...
FL_TEST_CASE("Corkscrew caching functionality") {
    ...
    fl::Tile2x2_u8_wrap tile1 = corkscrew.at_wrap(1.0f);
    fl::Tile2x2_u8_wrap tile1_again = corkscrew.at_wrap(1.0f);
    ...
    for (int x = 0; x < 2; x++) {
        for (int y = 0; y < 2; y++) {
            ...
            FL_REQUIRE_EQ(data1.first.x, data1_again.first.x);
            FL_REQUIRE_EQ(data1.first.y, data1_again.first.y);
            FL_REQUIRE_EQ(data1.second, data1_again.second);
        }
    }
}
...
FL_TEST_CASE("Corkscrew caching disable functionality") {
    ...
    fl::Tile2x2_u8_wrap tile_cached = corkscrew.at_wrap(1.0f);
    
    // Disable caching
    corkscrew.setCachingEnabled(false);
    
    // Get the same tile with caching disabled
    fl::Tile2x2_u8_wrap tile_uncached = corkscrew.at_wrap(1.0f);
    ...
    for (int x = 0; x < 2; x++) {
        for (int y = 0; y < 2; y++) {
            ...
            FL_REQUIRE_EQ(data_cached.first.x, data_uncached.first.x);
            FL_REQUIRE_EQ(data_cached.first.y, data_uncached.first.y);
            FL_REQUIRE_EQ(data_cached.second, data_uncached.second);
        }
    }
    ...
    fl::Tile2x2_u8_wrap tile_recached = corkscrew.at_wrap(1.0f);
    ...
    for (int x = 0; x < 2; x++) {
        for (int y = 0; y < 2; y++) {
            ...
            FL_REQUIRE_EQ(data_cached.first.x, data_recached.first.x);
            FL_REQUIRE_EQ(data_cached.first.y, data_recached.first.y);
            FL_REQUIRE_EQ(data_cached.second, data_recached.second);
        }
    }
}
...
FL_TEST_CASE("Corkscrew caching with edge cases") {
    ...
    fl::Tile2x2_u8_wrap tile0 = corkscrew.at_wrap(0.0f);
    fl::Tile2x2_u8_wrap tile4 = corkscrew.at_wrap(4.0f);
    ...
    FL_REQUIRE(tiles_different); // Tiles at different positions should be different
    
    // Test that same index produces same tile (cache consistency)
    fl::Tile2x2_u8_wrap tile0_again = corkscrew.at_wrap(0.0f);
    for (int x = 0; x < 2; x++) {
        for (int y = 0; y < 2; y++) {
            auto data0 = tile0.at(x, y);
            auto data0_again = tile0_again.at(x, y);
            FL_REQUIRE_EQ(data0.first.x, data0_again.first.x);
            FL_REQUIRE_EQ(data0.first.y, data0_again.first.y);
            FL_REQUIRE_EQ(data0.second, data0_again.second);
        }
    }
}
