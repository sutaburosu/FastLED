...
FL_TEST_CASE("LUT interp8") {
    ...
    data[0] = 0;
    data[1] = 255;
    FL_CHECK_EQ(lut.interp8(0), 0);
    FL_CHECK_EQ(lut.interp8(255), 255);
    FL_CHECK_EQ(lut.interp8(128), 128);

    // Check the LUT interpolation for all values.
    for (uint16_t i = 0; i < 256; i++) {
        uint16_t expected = (i * 255) / 255;
        FL_CHECK_EQ(lut.interp8(i), expected);
    }
}
...
FL_TEST_CASE("LUT interp16") {
    ...
    data[0] = 0;
    data[1] = 255;
    FL_CHECK_EQ(lut.interp16(0), 0);
    FL_CHECK_EQ(lut.interp16(0xffff), 255);
    FL_CHECK_EQ(lut.interp16(0xffff / 2), 127);
