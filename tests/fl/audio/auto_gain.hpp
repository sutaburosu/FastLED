// Unit tests for audio::AutoGain - PI controller with peak envelope tracking
// standalone test
...
static audio::Sample createSample_AutoGain(const vector<i16>& samples, u32 timestamp = 0) {
...
FL_TEST_CASE("audio::AutoGain - amplification converges to target") {
    audio::AutoGain agc;
    ...
    audio::Sample audio = createSample_AutoGain(quiet, 1000);
...
FL_TEST_CASE("audio::AutoGain - attenuation converges to target") {
    audio::AutoGain agc;
    audio::AutoGainConfig config;
    config.enabled = true;
    config.targetRMSLevel = 8000.0f;
    ...
    audio::Sample audio = createSample_AutoGain(loud, 2000);
...
FL_TEST_CASE("audio::AutoGain - silence produces no NaN") {
    audio::AutoGain agc;
    audio::AutoGainConfig config;
    config.enabled = true;
    config.targetRMSLevel = 8000.0f;
    ...
    audio::Sample audio = createSample_AutoGain(silence, 3000);

    for (int i = 0; i < 20; ++i) {
        audio::Sample result = agc.process(audio);
...
FL_TEST_CASE("audio::AutoGain - gain clamping bounds") {
    // Test max gain clamping
    {
        ...
        audio::Sample audio = createSample_AutoGain(veryQuiet, 4000);
    ...
    // Test min gain clamping
    {
        ...
        audio::Sample audio = createSample_AutoGain(veryLoud, 5000);
        for (int i = 0; i < 100; ++i) agc.process(audio);
...
FL_TEST_CASE("audio::AutoGain - disabled passthrough") {
    audio::AutoGain agc;
    audio::AutoGainConfig config;
    ...
    audio::Sample input = createSample_AutoGain(samples, 8000);
    audio::Sample output = agc.process(input);
...
// Keep: Empty/invalid
FL_TEST_CASE("audio::AutoGain - empty and invalid") {
    audio::AutoGain agc;
    FL_CHECK_FALSE(agc.process(audio::Sample()).isValid());
...
// Keep: Timestamp
FL_TEST_CASE("audio::AutoGain - timestamp preserved") {
    audio::AutoGain agc;
    vector<i16> samples = generateConstantSignal(500, 5000);
    audio::Sample output = agc.process(createSample_AutoGain(samples, 123456));
    FL_CHECK_EQ(output.timestamp(), 123456u);
}

// Keep: Reset
FL_TEST_CASE("audio::AutoGain - reset clears state") {
    audio::AutoGain agc;
...
// Keep: No clipping
FL_TEST_CASE("audio::AutoGain - no clipping on extreme amplification") {
    audio::AutoGain agc;
    audio::AutoGainConfig config;
    ...
    audio::Sample audio = createSample_AutoGain(quiet, 9000);
...
FL_TEST_CASE("audio::AutoGain - presets produce different convergence speeds") {
    // Feed same quiet signal to Normal and Vivid, compare gain after 50 frames
    auto runWithPreset = [](audio::AGCPreset preset) -> float {
        audio::AutoGain agc;
        audio::AutoGainConfig config;
        ...
        vector<i16> quiet = generateConstantSignal(1000, 500);
        audio::Sample audio = createSample_AutoGain(quiet, 0);
...
FL_TEST_CASE("audio::AutoGain - slow convergence over first second") {
    ...
    audio::Sample audio = createSample_AutoGain(atTarget, 0);
...
FL_TEST_CASE("audio::AutoGain - cross-band stability with alternating levels") {
    ...
    audio::Sample loudAudio = createSample_AutoGain(loud, 0);
    audio::Sample quietAudio = createSample_AutoGain(quiet, 0);
...
FL_TEST_CASE("audio::AutoGain - cold start with loud signal") {
    audio::AutoGain agc;
    audio::AutoGainConfig config;
    config.targetRMSLevel = 8000.0f;
    agc.configure(config);
    ...
    audio::Sample audio = createSample_AutoGain(loud, 0);
...
FL_TEST_CASE("audio::AutoGain - cold start with silence then signal") {
    audio::AutoGain agc;
    audio::AutoGainConfig config;
    config.targetRMSLevel = 8000.0f;
    agc.configure(config);
    ...
    audio::Sample silenceAudio = createSample_AutoGain(silence, 0);
    ...
    audio::Sample signalAudio = createSample_AutoGain(signal, 0);
...
FL_TEST_CASE("audio::AutoGain - cold start gain is approximately unity") {
    audio::AutoGain agc;
    audio::AutoGainConfig config;
    config.targetRMSLevel = 8000.0f;
    agc.configure(config);
...
FL_TEST_CASE("audio::AutoGain - prolonged silence then loud burst") {
    audio::AutoGain agc;
    audio::AutoGainConfig config;
    config.targetRMSLevel = 8000.0f;
    agc.configure(config);
...
FL_TEST_CASE("audio::AutoGain - silence integrator spin-down") {
    audio::AutoGain agc;
    audio::AutoGainConfig config;
    config.targetRMSLevel = 8000.0f;
    agc.configure(config);
...
FL_TEST_CASE("audio::AutoGain - intermittent silence stability") {
    audio::AutoGain agc;
    audio::AutoGainConfig config;
    config.targetRMSLevel = 8000.0f;
    agc.configure(config);
...
FL_TEST_CASE("audio::AutoGain - sudden 100x level increase") {
    audio::AutoGain agc;
    audio::AutoGainConfig config;
    config.targetRMSLevel = 8000.0f;
    agc.configure(config);
...
FL_TEST_CASE("audio::AutoGain - sudden 100x level decrease") {
    audio::AutoGain agc;
    audio::AutoGainConfig config;
    config.targetRMSLevel = 8000.0f;
    agc.configure(config);
...
FL_TEST_CASE("audio::AutoGain - slowly rising noise floor") {
    audio::AutoGain agc;
    audio::AutoGainConfig config;
    config.targetRMSLevel = 8000.0f;
    agc.configure(config);
    ...
    for (int i = 0; i < 100; ++i) {
        ...
        audio::Sample audio = createSample_AutoGain(signal, 0);
        agc.process(audio);
    }
...
FL_TEST_CASE("audio::AutoGain - slowly falling noise floor") {
    audio::AutoGain agc;
    audio::AutoGainConfig config;
    config.targetRMSLevel = 8000.0f;
    agc.configure(config);
...
FL_TEST_CASE("audio::AutoGain - mic warmup exponential rise") {
    audio::AutoGain agc;
    audio::AutoGainConfig config;
    config.targetRMSLevel = 8000.0f;
    agc.configure(config);
...
FL_TEST_CASE("audio::AutoGain - integrator doesn't windup when clamped") {
    audio::AutoGain agc;
    audio::AutoGainConfig config;
    config.targetRMSLevel = 8000.0f;
    config.maxGain = 32.0f;
    ...
    audio::Sample quietAudio = createSample_AutoGain(veryQuiet, 0);
    for (int i = 0; i < 500; ++i) {
        agc.process(quietAudio);
    }
    ...
    audio::Sample normalAudio = createSample_AutoGain(normal, 0);
    for (int i = 0; i < 200; ++i) {
...
FL_TEST_CASE("audio::AutoGain - integrator recovers from saturation") {
    audio::AutoGain agc;
    audio::AutoGainConfig config;
    config.targetRMSLevel = 8000.0f;
    agc.configure(config);
    ...
    vector<i16> atTarget = generateConstantSignal(1000, 8000);
    audio::Sample targetAudio = createSample_AutoGain(atTarget, 0);
    for (int i = 0; i < 500; ++i) {
...
FL_TEST_CASE("audio::AutoGain - negative gain never produced") {
    audio::AutoGain agc;
    audio::AutoGainConfig config;
    config.targetRMSLevel = 8000.0f;
    agc.configure(config);
...
FL_TEST_CASE("audio::AutoGain - vivid converges faster than normal") {
    auto framesTo10Pct = [](audio::AGCPreset preset) -> int {
        audio::AutoGain agc;
        audio::AutoGainConfig config;
        config.preset = preset;
        config.targetRMSLevel = 8000.0f;
...
FL_TEST_CASE("audio::AutoGain - all presets stable under alternating input") {
    auto measureVariance = [](audio::AGCPreset preset) -> float {
        audio::AutoGain agc;
        audio::AutoGainConfig config;
        config.preset = preset;
        config.targetRMSLevel = 8000.0f;
...
FL_TEST_CASE("audio::AutoGain - custom preset matches manual values") {
...
FL_TEST_CASE("audio::AutoGain - single-sample frame") {
    audio::AutoGain agc;
    audio::AutoGainConfig config;
    config.targetRMSLevel = 8000.0f;
    agc.configure(config);
    ...
    audio::Sample audio = createSample_AutoGain(single, 0);
...
FL_TEST_CASE("audio::AutoGain - huge frame 10000 samples") {
    audio::AutoGain agc;
    audio::AutoGainConfig config;
    config.targetRMSLevel = 8000.0f;
    agc.configure(config);
    ...
    audio::Sample audio = createSample_AutoGain(huge, 0);
...
FL_TEST_CASE("audio::AutoGain - very low sample rate") {
    audio::AutoGain agc;
    audio::AutoGainConfig config;
    config.targetRMSLevel = 8000.0f;
    agc.configure(config);
