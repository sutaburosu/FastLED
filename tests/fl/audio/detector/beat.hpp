// Unit tests for audio::detector::Beat
...
static audio::Sample makeSample_BeatDetector(float freq, fl::u32 timestamp, float amplitude = 16000.0f) {
    return makeSample(freq, timestamp, amplitude);
}

static audio::Sample makeSilence_BeatDetector(fl::u32 timestamp) {
...
FL_TEST_CASE("audio::detector::Beat - silence produces no beats") {
    audio::detector::Beat detector;
    int beatCount = 0;
    detector.onBeat.add([&beatCount]() { beatCount++; });

    for (int i = 0; i < 20; ++i) {
        auto ctx = fl::make_shared<audio::Context>(makeSilence_BeatDetector(i * 23));
...
FL_TEST_CASE("audio::detector::Beat - strong bass onset after silence triggers beat") {
    audio::detector::Beat detector;
    detector.setThreshold(0.1f);  // Lower threshold for testing
    int beatCount = 0;
    detector.onBeat.add([&beatCount]() { beatCount++; });

    auto ctx = fl::make_shared<audio::Context>(makeSilence_BeatDetector(0));
    ctx->setSampleRate(44100);
...
FL_TEST_CASE("audio::detector::Beat - pure treble should not trigger beat") {
    audio::detector::Beat detector;
    int beatCount = 0;
    detector.onBeat.add([&beatCount]() { beatCount++; });
...
FL_TEST_CASE("audio::detector::Beat - getPhase returns valid range") {
    audio::detector::Beat detector;
    auto ctx = fl::make_shared<audio::Context>(makeSilence_BeatDetector(0));
    ctx->setSampleRate(44100);
    detector.update(ctx);
...
FL_TEST_CASE("audio::detector::Beat - getConfidence returns valid range") {
    audio::detector::Beat detector;
    auto ctx = fl::make_shared<audio::Context>(makeSilence_BeatDetector(0));
    ctx->setSampleRate(44100);
    detector.update(ctx);
...
FL_TEST_CASE("audio::detector::Beat - reset clears state") {
    audio::detector::Beat detector;
    auto ctx = fl::make_shared<audio::Context>(makeSilence_BeatDetector(0));
    ctx->setSampleRate(44100);
...
FL_TEST_CASE("audio::detector::Beat - needsFFT and needsFFTHistory") {
    audio::detector::Beat detector;
    FL_CHECK_TRUE(detector.needsFFT());
    FL_CHECK_TRUE(detector.needsFFTHistory());
}

FL_TEST_CASE("audio::detector::Beat - periodic bass onsets converge BPM") {
    audio::detector::Beat detector;
...
FL_TEST_CASE("audio::detector::Beat - phase increases monotonically between beats") {
    audio::detector::Beat detector;
    detector.setThreshold(0.1f);
...
FL_TEST_CASE("audio::detector::Beat - amplitude sweep: onset detection across dB levels") {
    for (const auto& level : kBeatAmplitudes) {
        ...
        if (level.amplitude >= 10000.0f) {
            ...
            FL_MESSAGE("audio::detector::Beat amplitude sweep [" << level.label
                       << " amp=" << level.amplitude
                       << "]: beat=" << (gotBeat ? "yes" : "no")
...
FL_TEST_CASE("audio::detector::Beat - amplitude sweep: periodic beats converge BPM") {
...
static audio::Sample makeBassSync(u32 timestamp, float amplitude = 20000.0f) {
    ...
    float threshold = 1.3f, float sensitivity = 1.0f)
{
    ...
    fl::vector<fl::i16> silenceData(512, 0);
    auto ctx = fl::make_shared<audio::Context>(audio::Sample(silenceData, 0));
    ctx->setSampleRate(44100);
    ctx->setFFTHistoryDepth(4);
...
FL_TEST_CASE("BeatSynthetic - beats detected over background noise") {
    ...
    const u32 frameMs = 23;
    const int framesPerBeat = 22;
    auto ctx = fl::make_shared<audio::Context>(audio::Sample());
    ctx->setSampleRate(44100);
    ctx->setFFTHistoryDepth(4);

    // Helper: generate bass + background sample
    auto makeBackgroundWithBass = [](u32 ts, bool addBass) -> audio::Sample {
...
FL_TEST_CASE("BeatSynthetic - kick triggers beat, snare does not") {
    ...
    auto makeSnareSignal = [](u32 ts) -> audio::Sample {
        fl::vector<fl::i16> data;
        data.reserve(512);
        for (int s = 0; s < 512; ++s) {
            float t = static_cast<float>(s) / 44100.0f;
...
// Test: Full audio::Processor pipeline with signal conditioning
// The audio::Processor wraps audio::detector::Beat but adds signal conditioning
// (DC removal, spike filter, noise gate) which may affect beat signals.
FL_TEST_CASE("BeatSynthetic - audio::Processor pipeline detects periodic beats") {
    ...
    for (int i = 0; i < 43; ++i) {
        timestamp += frameMs;
        processor.update(audio::Sample(silenceData, timestamp));
    }
    ...
    FL_MESSAGE("audio::Processor pipeline: " << beatCount << "/16 beats, BPM=" << lastBPM);
...
FL_TEST_CASE("BeatSynthetic - beats detected in noisy signal") {
    ...
    auto makeNoisySample = [](u32 ts, int seed, bool addBeat) -> audio::Sample {
...
FL_TEST_CASE("BeatSynthetic - realistic kick envelope detected") {
    ...
    auto makeKickFrame = [&](u32 ts) -> audio::Sample {
        fl::vector<fl::i16> data;
        data.reserve(512);
        for (int s = 0; s < 512; ++s) {
    ...
    for (int i = 0; i < 43; ++i) {
        ...
        ctx->setSample(audio::Sample(silence, timestamp));
        detector.update(ctx);
        detector.fireCallbacks();
    }
...
FL_TEST_CASE("BeatSynthetic - low SNR beats (2x above background)") {
    ...
    auto makeLowSNR = [](u32 ts, int seed, bool addKick) -> audio::Sample {
...
FL_TEST_CASE("BeatSynthetic - CQ bin coverage for kick drum frequencies") {
    ...
    audio::fft::FFT fft;
    fft.run(silence, &bins, audio::fft::Args(512, 16, 30.0f, audio::fft::Args::DefaultMaxFrequency(), 44100));
