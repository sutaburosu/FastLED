...
// Singleton audio::Context reused across all test cases. The audio::fft::FFT kernel cache
// (128-band CQ kernels = 128 audio::fft::FFT operations) is expensive to generate.
// Sharing one audio::Context avoids regenerating kernels for every test case.
static fl::shared_ptr<audio::Context>& sharedAudioContext() {
    static fl::shared_ptr<audio::Context> ctx;
    return ctx;
}

static fl::shared_ptr<audio::Context> getOrCreateContext(const audio::Sample& sample, int sampleRate) {
    auto& ctx = sharedAudioContext();
    if (!ctx) {
        ctx = fl::make_shared<audio::Context>(sample);
...
// Scope guard that clears audio::Context history on test exit,
// preventing state leakage between test cases while preserving
// the expensive audio::fft::FFT kernel cache.
...
static fl::vector<fl::i16> decodeOggToPcm(const char* path, fl::u32* outSampleRate = nullptr) {
    ...
    // Decode all samples
    fl::vector<audio::Sample> samples = Vorbis::decodeAll(oggData, &error);
    if (samples.empty()) {
        return fl::vector<fl::i16>();
    }
...
// --- Utility: Run audio::detector::Vocal over PCM stream ---
...
static RealAudioResult runDetectorOnPcm(fl::span<const fl::i16> pcm, int sampleRate = 44100, float maxSeconds = 3.0f) {
    ...
    for (fl::size offset = 0; offset + FRAME_SIZE <= maxSamples; offset += FRAME_SIZE) {
        ...
        audio::Sample sample(frame, timestamp);
...
FL_TEST_CASE("audio::detector::Vocal real-audio - vocal_solo detected") {
    VOCAL_TEST_CLEANUP;
...
FL_TEST_CASE("audio::detector::Vocal real-audio - mix_vocal_loud higher than backing") {
    VOCAL_TEST_CLEANUP;
    if (!fixturesAvailable()) {
...
FL_TEST_CASE("audio::detector::Vocal real-audio - guitar_solo not vocal") {
    VOCAL_TEST_CLEANUP;
    if (!fixturesAvailable()) {
        FL_MESSAGE("Skipping: OGG fixtures not found.");
        return;
...
FL_TEST_CASE("audio::detector::Vocal real-audio - drums_solo not vocal") {
    VOCAL_TEST_CLEANUP;
    if (!fixturesAvailable()) {
        FL_MESSAGE("Skipping: OGG fixtures not found.");
        return;
...
FL_TEST_CASE("audio::detector::Vocal real-audio - backing_only not vocal") {
    VOCAL_TEST_CLEANUP;
    if (!fixturesAvailable()) {
        FL_MESSAGE("Skipping: OGG fixtures not found.");
        return;
...
FL_TEST_CASE("audio::detector::Vocal real-audio - mix gradient") {
    VOCAL_TEST_CLEANUP;
    if (!fixturesAvailable()) {
        FL_MESSAGE("Skipping: OGG fixtures not found.");
        return;
...
FL_TEST_CASE("audio::detector::Vocal real-audio - cpp-side stem mixing") {
    VOCAL_TEST_CLEANUP;
    if (!fixturesAvailable()) {
        FL_MESSAGE("Skipping: OGG fixtures not found.");
        return;
...
FL_TEST_CASE("audio::detector::Vocal real-audio - feature diagnostics") {
    VOCAL_TEST_CLEANUP;
    if (!fixturesAvailable()) {
        FL_MESSAGE("Skipping: OGG fixtures not found.");
        return;
    ...
    for (const auto& fix : fixtures) {
        ...
        for (fl::size offset = 0; offset + FRAME_SIZE <= pcm.size() && offset + FRAME_SIZE <= MAX_DIAG_SAMPLES; offset += FRAME_SIZE) {
            fl::span<const fl::i16> frame(pcm.data() + offset, FRAME_SIZE);
            audio::Sample sample(frame, static_cast<fl::u32>(frameCount * 12));
...
FL_TEST_CASE("audio::detector::Vocal real-audio - voice stem decodes cleanly") {
    if (!fixturesAvailable()) {
        FL_MESSAGE("Skipping: OGG fixtures not found.");
        return;
    }
