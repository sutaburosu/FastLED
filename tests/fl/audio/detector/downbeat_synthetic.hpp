// Unit tests for audio::detector::Downbeat with synthetic data validation
...
// Helper: Create audio::Context with synthetic audio designed to trigger beat
// detection The audio::detector::Beat uses spectral flux (change in audio::fft::FFT magnitudes
...
shared_ptr<audio::Context> createMockAudioContext(u32 timestamp, bool isDownbeat,
    ...
    audio::Sample sample(pcmData, timestamp);
    auto context = make_shared<audio::Context>(sample);

    // Pre-compute audio::fft::FFT so it's available during update
    context->getFFT16();
...
DetectionMetrics runDetectorTest(audio::detector::Downbeat& detector,
    ...
    if (groundTruth.size() >= 2) {
        ...
        if (enableLogging) {
            FL_MESSAGE("Detected beat interval from ground truth: " << beatInterval << "ms (" << (60000.0f / beatInterval) << " BPM)");
        }
    }

    // Phase 1: Warm-up period to let detector establish rhythm
    // This allows audio::detector::Beat and audio::detector::Downbeat to build up history
...
FL_TEST_CASE("audio::detector::Downbeat - Basic downbeat pattern detection") {
    ...
    audio::detector::Downbeat detector;
    ...
    // This creates spectral flux that audio::detector::Beat can detect
    u32 timestamp = 0;
    u32 frameInterval = 23; // ~43 fps
    int downbeatCount = 0;
...
FL_TEST_CASE("audio::detector::Downbeat - Meter setting and beat counting") {
...
FL_TEST_CASE("audio::detector::Downbeat - Confidence bounds") {
...
FL_TEST_CASE("audio::detector::Downbeat - Measure phase tracking") {
...
FL_TEST_CASE("audio::detector::Downbeat - Basic functionality") {
    ...
    audio::Sample sample(pcmData, 1000);
    auto context = make_shared<audio::Context>(sample);
...
FL_TEST_CASE("audio::detector::Downbeat - Strong accent strength (2x multiplier)") {
...
FL_TEST_CASE("audio::detector::Downbeat - Weak accent strength (0.6x multiplier)") {
...
FL_TEST_CASE("audio::detector::Downbeat - Timing jitter tolerance") {
...
FL_TEST_CASE("audio::detector::Downbeat - Confidence threshold impact") {
    ...
    audio::detector::Downbeat detector1;
    ...
    audio::detector::Downbeat detector2;
...
FL_TEST_CASE("audio::detector::Downbeat - 3/4 waltz pattern") {
...
FL_TEST_CASE("audio::detector::Downbeat - Confidence analysis: strong vs weak accents") {
...
FL_TEST_CASE("audio::detector::Downbeat - No time-skipping comparison") {
