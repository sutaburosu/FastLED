// Unit tests for audio::detector::FrequencyBands — 3-band frequency detector with tone sweep validation
...
FL_TEST_CASE("audio::detector::FrequencyBands - tone sweep shows continuous band motion") {
    // Sweep a tone from 100 Hz to 8000 Hz. The audio::detector::FrequencyBands detector has:
...
FL_TEST_CASE("audio::detector::FrequencyBands - tone sweep has no jitter in raw band energy") {
    ...
    // smooth. Allow 0.5 threshold (same as audio::detector::Vibe).
    FL_CHECK_LT(bassJitter, 0.5f);
    FL_CHECK_LT(midJitter, 0.5f);
    FL_CHECK_LT(trebJitter, 0.5f);
}

FL_TEST_CASE("audio::detector::FrequencyBands - tone sweep normalized levels track smoothly") {
